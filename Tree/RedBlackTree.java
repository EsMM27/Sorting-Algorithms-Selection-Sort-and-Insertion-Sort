...
public class RedBlackTree<T extends Comparable<T>> {
	...
	protected void replaceParentChild(Node oldRoot, Node newRoot) {
		...
		System.out.println(">>> replaceParentChild: oldRoot=" + oldRoot.value + 
				", newRoot=" + (newRoot != null ? newRoot.value : "null") + 
				", parent=" + (p != null ? p.value : "null"));
		if (p == null) {
			root = newRoot;
			if (newRoot != null) newRoot.parent = null;
			System.out.println("    Updated tree root to " + (root != null ? root.value : "null"));
		} else if (p.left == oldRoot) {
			p.left = newRoot;
			if (newRoot != null) newRoot.parent = p;
			System.out.println("    Updated parent's LEFT child to " + (newRoot != null ? newRoot.value : "null"));
		} else {
			p.right = newRoot;
			if (newRoot != null) newRoot.parent = p;
			System.out.println("    Updated parent's RIGHT child to " + (newRoot != null ? newRoot.value : "null"));
		}
	}
